/*
# Atelier — Skill Exchange Platform Schema

1. Overview
- A multi-user platform where people exchange skills and knowledge.
- Users create profiles, list skills they can teach and skills they want to learn,
  propose exchanges with other users, and leave reviews after completed exchanges.
...
- `profiles`
  ...
  - `bio` (text)
  - `location` (text)
  - `avatar_color` (text — a palette key used to render a generated avatar)
...
- `exchanges`
  ...
  - `skill_id` (uuid, references skills — the skill being requested)
  - `message` (text)
  - `status` (text: 'pending' | 'accepted' | 'declined' | 'completed', default 'pending')
  - `created_at` (timestamptz)
  - `updated_at` (timestamptz)
- `reviews`
  - `id` (uuid, PK)
  - `exchange_id` (uuid, references exchanges)
  - `reviewer_id` (uuid, references profiles, defaults to auth.uid())
  - `rating` (int, 1-5)
  - `comment` (text)
  - `created_at` (timestamptz)
...
- profiles: each user can read all profiles (discovery), update only their own.
- skills: all authenticated users can read (discovery); users manage only their own.
- exchanges: users can read exchanges where they are requester or recipient;
  requesters create/update their own; recipients update status.
- reviews: all authenticated users can read; only the reviewer creates/updates their own.

4. Important Notes
- Owner columns default to auth.uid() so frontend inserts that omit user_id still pass RLS.
- profiles are created via a trigger on auth.users signup OR by the frontend on first login.
*/
